...
//		 * 这是spark本身提供的特性
		...
		//这些actionRDD，sessionid2actionRDD在多处要被其他函数使用，所以写在main方法内做出公共RDD（重构），可减少RDD的生成，减少资源使用
...
//		sessionid2actionRDD.checkpoint();
		...
		//Accumulator<String>累加器，<String>是累加器要操作的类型。sc.accumulator("", new SessionAggrStatAccumulator())，这里要累加的逻辑是我们自定义的，一开始初始化为空""，new SessionAggrStatAccumulator()就是我们自己定义累加器要计算的逻辑（如果不自己去实现这个逻辑，默认是阿拉伯数字累加）
		...
		//下面具体实现的函数中，传入的参数是sessionid2AggrInfoRDD和sessionid2actionRDD，怎么回事？
		 ...
		 * 因为随机抽取功能中，有一个countByKey算子，是action操作，会触发job（触发了不好吗？accumulator就可以执行了啊？）
		 */
	...
	/**
	 * 目的：你要获取什么内容的RDD，它是什么格式的（JAVARDD，JAVAPairRDD？），你获得它后下一步是做什么用；
	 * 手段：你怎么获得它？获得它需要传入什么参数，需要用到什么算子，要处理哪些逻辑
	 * 获取指定日期范围内的用户访问行为数据
...
//		return actionDF.javaRDD().repartition(1000);
	...
	/**
	 * 目地：你要获取什么内容的RDD，它是什么格式的（JAVARDD，JAVAPairRDD？），你获得它后下一步是做什么用（不要只想这一步得到这个，要想得到这个下一步和最终是要什么）；
	 * 手段：你怎么获得它？获得它需要传入什么参数，需要用到什么算子，要处理哪些逻辑
...
//			
//		});
		//真正知识细下来，这里就不清楚为什么要使用这个算子了（因为考虑到本地生产的模拟数据量不大，为了加快处理速度，直接用mapPartitionsToPair;mapPartitionsToPair与mapPartitions不同就是一个处理出来结果JavaPairRDD，一个处理出来结果为JavaRDD）
		return actionRDD.mapPartitionsToPair(new PairFlatMapFunction<Iterator<Row>, String, Row>() {

			private static final long serialVersionUID = 1L;

			@Override
			//传入Iterator<Row> iterator一次性批量处理，传出Iterable<Tuple2<String, Row>>，一次性批量得到结果
	...
	private static JavaPairRDD<String, String> aggregateBySession(
		...
		// 对每一个session分组进行聚合，将session中所有的搜索词和点击品类都聚合起来（得到每个sessionid的所有搜索词和点击品类，这一个是当前目的；最后目的是为了得到每个sessionid的一份完整“信息”，然后使用大数据平台的人，可以通过设置目标信息和之前每个sessionid的完整信息进行匹配，从而过滤数据）
		...
		JavaPairRDD<Long, String> userid2PartAggrInfoRDD = sessionid2ActionsRDD.mapToPair(
				...
				new PairFunction<Tuple2<String,Iterable<Row>>, Long, String>() {
					...
					public Tuple2<Long, String> call(Tuple2<String, Iterable<Row>> tuple)
						...
						//函数中接收的传入数据，是每次传入一条已经聚合过的Tuple2<String, Iterable<Row>>，所以userid每次进来都新定义，stepLength步长值也被初始化为0
						Long userid = null;
						...
						while(iterator.hasNext()) {
							...
							//每个sessionid的数据，一开始进来的第一条session，开始和结束时间是一样的（starttime和endtime都是空）；后续再传进来相同sessionid下的其他session，actionTime就不同了
							if(startTime == null) {
						...
						//这里的返回是针对每一个userid的，多个userid会返回多次，组合成JavaPairRDD<Long, String> userid2PartAggrInfoRDD
		...
		//这里最后直接调用javaRDD()就返回JavaRDD<Row>；否者会先返回Dataframe
		...
		//最后目的是为了得到一个sessionid对应的完整“信息”，便于过滤
...
//				});
		...
		//JavaPairRDD<Long, Long> reversedSampledRDD =  new Tuple2<Long, Long>(汇总, sessionid);
		...
		//倒叙排列，取出一条数据（此时取出这一条就是第一位的）（就是采样数据最多的那个sessionid，最可能发生数据倾斜的那个），get(0)是获取list第一个元素，这个元素是个tuple，再获取tuple的第二个元素，就是sessionid，用final把它做成常量
...
//				});
		...
		JavaPairRDD<String, String> filteredSessionid2AggrInfoRDD = sessionid2AggrInfoRDD.filter(
				...
				new Function<Tuple2<String,String>, Boolean>() {
					...
					public Boolean call(Tuple2<String, String> tuple) throws Exception {
						...
						//这里累加session做什么用？（后面用来算步长和访问时长的占比）
	...
	private static void calculateAndPersistAggrStat(String value, long taskid) {
		...
		//插入封装好的domain对象（就是那个javabean）
		sessionAggrStatDAO.insert(sessionAggrStat);  
	}
	
	/**
	 * 
	 * 获取通过筛选条件的（这里只通过时间筛选条件了？其它筛选条件没考虑？）session的访问明细数据RDD
	 * @param sessionid2aggrInfoRDD
	 * @param sessionid2actionRDD
	 * @return
	 */
	//sessionid2aggrInfoRDD就是按用户指定条件过滤后的完整数据？<sessionid,(sessionid,searchKeywords,clickCategoryIds,age,professional,city,sex)>
	...
	//最后返回的数据以main函数中传入的参数为准，这里的这个函数里的参数做什么用呢？
	...
	private static void randomExtractSession(
		...
		//Map<String, Object> countMap=<yyyy-MM-dd_HH,对应某小时下所有aggrInfo的聚合数量>
		Map<String, Object> countMap = time2sessionidRDD.countByKey();
		
		/**
		 * 第二步，使用按时间比例随机抽取算法，计算出每天每小时要抽取session的索引（算出每天每小时要抽取的session总量，然后以这个总量的数值为范围进行随机抽取，然后把这些随机值加入到一个list中，构成随机抽取索引值数组）
		 */
		...
		//@@提取每小时session数量，Map<String, Long> hourCountMap = map<HH,count>
		...
		Random random = new Random();
		//遍历Map<String, Map<String, Long>> dateHourCountMap；dateHourCountMap这些变量命名都是有要求的，含义清晰，且驼峰式写法
		for(Map.Entry<String, Map<String, Long>> dateHourCountEntry : dateHourCountMap.entrySet()) {
			...
			//遍历hourCountMap的value值，把它们累加（把当天每个小时的加起来就是一天的总量）
			...
			for(Map.Entry<String, Long> hourCountEntry : hourCountMap.entrySet()) {
				...
				//如果用户所要当前小时要抽取的量，比当前小时的总量还要大的话，直接返回
...
//		});
	...
	private static List<Tuple2<CategorySortKey, String>> getTop10Category(  
		...
		JavaPairRDD<Long, Long> categoryidRDD = sessionid2detailRDD.flatMapToPair(
				...
				new PairFlatMapFunction<Tuple2<String,Row>, Long, Long>() {
					...
					public Iterable<Tuple2<Long, Long>> call(
						...
						//参考MockData里的"user_visit_action"表，点击过的id；一次点击，只有一个品类
						Long clickCategoryId = row.getLong(6);
						if(clickCategoryId != null) {
							list.add(new Tuple2<Long, Long>(clickCategoryId, clickCategoryId));   
						}
						//下过单的品类id，一次下单可能有多个品类，用的数组
		 ...
		 * 最后很可能会拿到重复的数据
		 */
		//categoryid有重复的，有些点击品类同一个商品点击多次的或者下单多次的都是有可能的（代码要和业务结合的很深，运行代码过程中从结果来调bug，否则一些代码真的很难理解）
				...
				new PairFunction<Tuple2<Long,String>, CategorySortKey, String>() {
					...
					public Tuple2<CategorySortKey, String> call(
						...
						CategorySortKey sortKey = new CategorySortKey(clickCount,
								orderCount, payCount);
						//key是sortKey，传入的是CategorySortKey对象类型，按key进行排序；countInfo是各品类的点击，下单，支付信息
		...
		//自定义了public class CategorySortKey implements Ordered<CategorySortKey>，继承接口Ordered<CategorySortKey>，再把这个接口内部的方法全部自己实现出来，这样就实现自己定的二次排序逻辑，作用到sortByKey算子上
		...
		//取出排名前10的List<Tuple2<CategorySortKey, String>> top10CategoryList
		List<Tuple2<CategorySortKey, String>> top10CategoryList = 
		...
		for(Tuple2<CategorySortKey, String> tuple: top10CategoryList) {
			...
			//调用ITop10CategoryDAO接口的实现类Top10CategoryDAOImpl的insert（）方法，并把category对象传进去
...
//				.coalesce(100);  
		 ...
		 * 
		 */
		//点击品类也可能是多个，为什么用mapToPair而不用faltMapToPair（从业务数据看，一条session只记录一个点击品类，所以用mapToPair;一般flatMap处理那种iterator类型的数据）
		JavaPairRDD<Long, Long> clickCategoryIdRDD = clickActionRDD.mapToPair(
				...
				new PairFunction<Tuple2<String,Row>, Long, Long>() {
					...
					public Tuple2<Long, Long> call(Tuple2<String, Row> tuple)
						...
						//这里的1L是指的clickCategoryId一次点积累量，后面用来聚合
...
//				});
	...
	private static JavaPairRDD<Long, Long> getOrderCategoryId2CountRDD(
		...
		//这里用flatMapToPair算子，因为客户可能会对多个品类下单；为什么上面的点击品类也可能是多个，却用mapToPair？（这里就涉及到业务细节，一次点击记录的点击品类只有一个，所以用mapToPair；一个session记录却可出现同时对多个品类下单，可以用flatMapToPair）
	...
	private static JavaPairRDD<Long, String> joinCategoryAndData(
		...
		//为什么老师一开始就知道join后的类型是JavaPairRDD<Long, Tuple2<Long, Optional<Long>>>这种形式的？（因为是join算子啊）
		//！！通过leftOuterjoin就可以把categoryid中的元素所有都留下，因为点击，下单或者支付的id，这三个都是categoryid的子集；点击，下单或者支付的id连续通过三次和categoryid进行join后，所有点击，下单或者支付的id都可以得到计算，如果直接用join，第一次和点击品类join后，部分下单和支付的id可能就被过滤掉了，因为有些品类时只点击，没有下单或购买的
	...
	private static void getTop10Session(
		...
		for(Tuple2<CategorySortKey, String> category : top10CategoryList) {
			//从上一个功能模块得value = value + "|" + Constants.FIELD_PAY_COUNT + "=" + payCount;  
			long categoryid = Long.valueOf(StringUtils.getFieldFromConcatString(
		...
		JavaPairRDD<Long, String> categoryid2sessionCountRDD = sessionid2detailsRDD.flatMapToPair(
				...
				new PairFlatMapFunction<Tuple2<String,Iterable<Row>>, Long, String>() {
					...
					public Iterable<Tuple2<Long, String>> call(
						...
						// 计算出该session（注意这里的逻辑是针对每条不同session的，flatMapToPair），对每个品类的点击次数
						//遍历每个sessionid对应的row,算出对每个品类的点击次数
						while(iterator.hasNext()) {
							Row row = iterator.next();
						...
						// 返回结果，<categoryid,sessionid,count>格式(String value = sessionid + "," + count;)
		...
		//JavaPairRDD<Long, Long> top10CategoryIdRDD = Tuple2<Long, Long>(categoryid, categoryid)
		//JavaPairRDD<Long, String> categoryid2sessionCountRDD=Tuple2<Long, String>(categoryid, value--->sessionid,count))
		...
		/**
		 * 第三步：分组取TopN算法实现，获取每个品类的top10活跃用户（拿到每个top10品类点击前十位的用户）
		 */
		...
		JavaPairRDD<String, String> top10SessionRDD = top10CategorySessionCountsRDD.flatMapToPair(
				...
				new PairFlatMapFunction<Tuple2<Long,Iterable<String>>, String, String>() {
					...
					public Iterable<Tuple2<String, String>> call(
							...
							throws Exception {
						long categoryid = tuple._1;
						//每个品类被各个session点击的次数，JavaPairRDD<品类id, Iterable<value--->sessionid,count>> top10CategorySessionCountsRDD取tuple._2.iterator();
						Iterator<String> iterator = tuple._2.iterator();
						...
						while(iterator.hasNext()) {
							...
							//从Iterable<value--->sessionid,count>按逗号分隔，取出第二个字段的内容
							long count = Long.valueOf(sessionCount.split(",")[1]);  
							
							// 遍历排序数组
							//这里就涉及一些算法思想了，训练的少，这些算法代码就很难理解（老师原话：算法很难讲解，只能意会，不能言传）
							//其实这里就是实现一个排序算法，要知道这个算法的目的，才有方向去看每行代码意图
